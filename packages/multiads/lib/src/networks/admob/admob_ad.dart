...
class AdmobAD extends Ads {
  ...
  void _finishAppOpen() {
    final done = AdCallbacks.onAppOpenFinished;
  ...
  Future<void> loadAppOpenAd() async {
    ...
    await AppOpenAd.load(
      ...
      adLoadCallback: AppOpenAdLoadCallback(
        onAdLoaded: (ad) {
          _appOpenAd = ad;
          Log.log('Admob >> App open ad loaded');
        },
        onAdFailedToLoad: (error) {
          _appOpenAd = null;
          Log.log('Admob >> App open ad failed to load: ${error.message}');
        },
      ),
      request: const AdRequest(),
    );
  ...
  void showAdIfAvailableOpenAds() {
    ...
    Log.log('Admob >> Showing app open ad');
    ad.fullScreenContentCallback = FullScreenContentCallback(
      onAdShowedFullScreenContent: (_) {
        isInterShowed = true;
        Log.log('Admob >> App open ad showed');
      },
      onAdFailedToShowFullScreenContent: (failedAd, error) {
        Log.log('Admob >> App open ad failed to show: $error');
        failedAd.dispose();
        loadAppOpenAd();
        isInterShowed = false;
        _finishAppOpen();
      ...
      onAdDismissedFullScreenContent: (dismissedAd) {
        Log.log('Admob >> App open ad dismissed');
        dismissedAd.dispose();
        loadAppOpenAd();
        isInterShowed = false;
        _finishAppOpen();
  ...
  Widget getBannerAdWidget(Key key) {
    final entry = _banners[key];
    if (entry == null || !entry.isReady) return const SizedBox.shrink();
    Log.log("Admob >> banner is visible $key");
    return SizedBox(
      width: entry.bannerAd.size.width.toDouble(),
      height: entry.bannerAd.size.height.toDouble(),
      child: AdWidget(ad: entry.bannerAd),
    );
  }
